...
public:
	...
	void kfinit();
	void upPhi(CSINS& ins,double dt, int iType);
	void upHk(CSINS& ins, double Hk[]);
	void TUpdate(double dt, int opt, int iType);
	void MUpdate(CSINS& ins,double ZK[],int zflag = 0xffff);
...
public:
	...
	void upHk(CSINS& ins, double Hk[]);
	void TUpdate(double dt, int opt = 0);
	void MUpdate(CSINS& ins,double ZK[],int zflag = 0xffff);
	void Feedback(CSINS& ins,double scater=1.0);
};
