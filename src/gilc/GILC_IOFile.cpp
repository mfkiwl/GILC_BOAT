...
{
	char logfile[256],kfile[256],posLCfile[256],posGfile[256],posNmeafile[256],kferrfile[256];
	...
	if(bTmpFileSave)
	{
		...
		strcpy(kferrfile, "D:/TMP/"); strcat(kferrfile, "Err.txt");

		foutErr = fopen(kferrfile, "wt");
		if (!foutErr)
			gilc_log("open file err! %s\r\n", foutErr);
...
void printf_posLC(FILE *fp, CSINS *ins, CLCData *ilcd)
{
	...
	fprintf(fp, "%4d/%02d/%02d %02d:%02d:%06.3f %15.9f %15.9f %11.4f %4d %4d\n",
		int(ep[0]),int(ep[1]),int(ep[2]),int(ep[3]),int(ep[4]),ep[5],ins->pos[0]*R2D,ins->pos[1]*R2D,ins->pos[2],ilcd->stat, (ilcd->nsused>0 ? ilcd->nsused : 1));
}
...
void printf_posAnt(FILE *fp, CSINS *ins, CLCData *ilcd)
{
	...
	time2epoch(gt, ep);
	fprintf(fp, "%4d/%02d/%02d %02d:%02d:%06.3f %15.9f %15.9f %11.4f %4d %4d\n",
		int(ep[0]),int(ep[1]),int(ep[2]),int(ep[3]),int(ep[4]),ep[5],ins->posL[0]*R2D,ins->posL[1]*R2D,ins->posL[2],ilcd->stat, (ilcd->nsused>0 ? ilcd->nsused : 1));
}
...
void printf_posG(FILE *fp, CLCData *ilcd)
{
	...
	fprintf(fp,"%4d/%02d/%02d %02d:%02d:%06.3f %15.9f %15.9f %11.4f %4d %4d\n",
		int(ep[0]),int(ep[1]),int(ep[2]),int(ep[3]),int(ep[4]),ep[5],ilcd->pos[0]*R2D,ilcd->pos[1]*R2D,ilcd->pos[2],ilcd->stat, (ilcd->nsused>0?ilcd->nsused :1));
}
...
{
	if (!fp) return;
	double heading_ins = atan2(gipro->ins.vnL[0], gipro->ins.vnL[1])*R2D;
	if (heading_ins < 0) heading_ins += 360;
	...
	double ins_speed = sqrt(pow(gipro->ins.vm_car[0], 2) + pow(gipro->ins.vm_car[1], 2));
	double dUtcTime = (ilcd->ep[3] * 10000 + ilcd->ep[4] * 100 + ilcd->ep[5]);
	double gilc_speed= sqrt(pow(gipro->ins.vnL[0], 2) + pow(gipro->ins.vnL[1], 2));
	fprintf(fp, "%9.3f %9.3f %d %d %d %.9f %.9f %.9f %.3f %.3f %.3f %.3f %.3f %.3f %.9f %.9f %.9f %.3f %.3f %.3f %.6f %.6f %.6f %.6f %.6f %.6f ",
		...
		gipro->ins.posL[0] * R2D, gipro->ins.posL[1] * R2D, gipro->ins.posL[2],
	...
	fprintf(fp, "%.6f %.6f %.6f %.6f %.6f %.6f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.3f %.3f %d %d %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.6f %f %f\n",
		...
		gipro->ins.lever[1],
		gipro->ins.lever[2],
		gnss_speed,gilc_speed,
		gipro->bgnssStatic,gipro->iDriverMode,
		heading_gnss,heading_ins,ilcd->heading2,
		...
		gipro->ins.water_flow_speed, gipro->ins.vm_car[0]
	);
}


void printf_Err(FILE *fp, GIProcess *gipro, CLCData *ilcd)
{
	if (fabs(gipro->ins.dpos[0]) > 100) gipro->ins.dpos[0] = 0;
	if (fabs(gipro->ins.dpos[1]) > 100) gipro->ins.dpos[1] = 0;
	if (fabs(gipro->ins.dpos[2]) > 100) gipro->ins.dpos[2] = 0;
	if (!fp) return;
	fprintf(fp, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,\n", ilcd->imutimetarge,
		gipro->ins.dpos[0], gipro->ins.dpos[1], gipro->ins.dpos[2],
		gipro->ins.dvn[0], gipro->ins.dvn[1], gipro->ins.dvn[2],
		gipro->ins.dvm[0], gipro->ins.dvm[1], gipro->ins.dvm[2],
		gipro->ins.datt_fb[0], gipro->ins.datt_fb[1], gipro->ins.datt_fb[2],
		gipro->ins.dvn_fb[0], gipro->ins.dvn_fb[1], gipro->ins.dvn_fb[2],
		gipro->ins.dpos_fb[0], gipro->ins.dpos_fb[1], gipro->ins.dpos_fb[2],
		gipro->ins.deb_fb[0], gipro->ins.deb_fb[1],gipro->ins.deb_fb[2],
		gipro->ins.ddb_fb[0], gipro->ins.ddb_fb[1], gipro->ins.ddb_fb[2]);
}
