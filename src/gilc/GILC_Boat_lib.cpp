...
public:
	...
	int GILC_Rst_Filter(gilc_result_t* pstOut);//对地速、航向、位置去除误差大的点
	void GILC_Rst_Vel_smooth(GIProcess* gipro);//速度分量平滑
...
gilc_ret_e GilcProcess::GILC_LoadRaw_byStrn(char *buff,gilc_raw_t *pRaw)
{
	...
	else if (strstr(buff,"IMU,"))
	{
		...
		if(num == 9 || num == 10 || num == 11)
		{
			...
			pRaw->bPPSavail=((int)val[8]>0?true:false);// 根据标志位判断PPS
			if (num == 10)
...
/******************************原始数据质量检验**********************************/
	...
	{
		GILC_GnssRaw_Correct(pstRaw);//gnss定位状态修正		
	...
	{
		GILC_ImuAxis_Correct(pstRaw);//imu轴系调整 ENU（右前上）	
	...
	if(GetGPS)
	{
		int iStatic = 0;
			...
			if (ilcd.bGPSavail)
			{
				...
				{
					Raw_gnss_speed.erase(Raw_gnss_speed.begin());//窗口滑动
					Raw_gnss_speed.push_back(ilcd.gnss_speed);
					double mean_speed = GetAveStd(Raw_gnss_speed, 0);//计算窗口均值 
					if (ilcd.stat == 4)
					{
						if (mean_speed < 0.02)
						{
			...
			if(ilcd.bMEMSavail)
			{
				...
				{
					Rst_speed.erase(Rst_speed.begin());//窗口滑动
					Rst_speed.push_back(Rst_speed_pre);
					double mean_speed = GetAveStd(Rst_speed, 0);//计算窗口均值
					if (mean_speed < 0.05)
					{
						gilc_static = 1;
					}
		...
		if(!bAlign)
		{
			if(s_cfgdata.bEkfXUse)
			{
				...
				{/*双天线航向误差已标定*/
					...
					gipro.bDualAntCalibrateOk = 1;//配置文件标定成功1 
				}
...
/******************************初始对准完成，进入算法解算********************************/
		...
		{
			int stat = 0;
			//double boat_yaw = gipro.ins.att_car[2] * R2D;//船头航向
			...
			//GILC_Rst_Vel_smooth(&gipro);//速度分量平滑
/***********************************debug组合中间结果保存*****************************************/
...
/*************************************组合结果状态判定******************************************/
//安装误差标定成功+双天线标定成功==iGilcStatus = GILC_RET__RST_STABLE
//gnss中断：iGilcStatus = GILC_RET__RST_INS_MODE
	if(!gipro.iGilcRunMode)//iGilcRunMode=0,不需要标定
	{
		if(iGilcStatus >= GILC_RET__RST_RESOLVING)
		{/*正常启动模式（已经标定），输出状态-组合模式*/
			if (gipro.bGnssLost)
...
/*************************************组合结果保存*****************************************/
//位置：deg  高程 ：m
//姿态：deg  姿态航向：-180~180deg（北偏西）  双天线航向：0~360deg（北偏东）
//角速度：经过安装误差、零偏补偿 deg
//加速度：经过安装误差、零偏补偿 g
		...
		if (iGilcStatus >= GILC_RET__RST_RESOLVING)
		{
			...
			if (s_cfgdata.iOutReferPoint == GILC_OUTREFER_POINT__GNSS)
			{
				...
				if (pstRaw->gnssdata.stat == 4)
				{
					...
					GILC_Rst_Filter(pstOut);//高程数据平滑
				}
				else
				{
					pstOut->lla[2] = gipro.ins.posL[2];
				}
				if (gipro.bgnssStatic)//gnss固定时，静态输出gnss位置
				{
			...
			//GILC_Rst_Filter(pstOut);//航向、速度平滑
