...
static double GetAveStd(vector<double, malloc_allocator<double> > a,int opt)
{
	int n=a.size();
...
{	
	IIR_Lowpass_Oder3(dOut, dIn, dIIR_Order3_bw_fs_0_5, 6);
...
#endif
int DetectStatic::DetectStatic_byOdo(double dWheelVel,int num)
{
...
int DetectStatic::DetectStatic_car_dsf(double ep[6],double acc[3],double gyo[3],double gpsvel[3],double insvel[3],int bGPSavail,int bGnssLost,int bGnssState)
{
#define NUM           500
