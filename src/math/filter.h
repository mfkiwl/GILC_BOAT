...
public:
	...
	vector<double, malloc_allocator<double> > axyz[3],gxyz[3],axyz_iir[3],gxyz_iir[3];
	vector<int, malloc_allocator<int> > vWheelVel;
	...
	int DetectStatic_byOdo(double dWheelVel,int num);
	int DetectStatic_car_dsf(double ep[6],double acc[3],double gyo[3],double gpsvel[3],double insvel[3],int bGPSavail,int bGnssLost,int bGnssState);
